...
if ( ! class_exists( 'SDS_Theme_Options' ) ) {
	...
	class SDS_Theme_Options {
		...
		function admin_init() {
			...
			// Logo
			add_settings_section( 'sds_theme_options_logo_section', __( 'Загрузка логотипа', 'capture'), array( $this, 'sds_theme_options_logo_section' ), 'sds-theme-options[general]' );
			add_settings_field( 'sds_theme_options_logo_field', __( 'Логотип:', 'capture'), array( $this, 'sds_theme_options_logo_field' ), 'sds-theme-options[general]', 'sds_theme_options_logo_section' );
			
			// Hide Tagline
			add_settings_section( 'sds_theme_options_hide_tagline_section', __( 'Показать / Скрыть Слоган сайта', 'capture'), array( $this, 'sds_theme_options_hide_tagline_section' ), 'sds-theme-options[general]' );
			add_settings_field( 'sds_theme_options_hide_tagline_field', __( 'Убрать - показать слоган:', 'capture'), array( $this, 'sds_theme_options_hide_tagline_field' ), 'sds-theme-options[general]', 'sds_theme_options_hide_tagline_section' );

			// Color Schemes (if specified by theme)
			if ( function_exists( 'sds_color_schemes' ) ) {
				add_settings_section( 'sds_theme_options_color_schemes_section', __( 'Цветовая схема', 'capture'), array( $this, 'sds_theme_options_color_schemes_section' ), 'sds-theme-options[general]' );
				add_settings_field( 'sds_theme_options_color_schemes_field', __( 'Выберите цветовую схему:', 'capture'), array( $this, 'sds_theme_options_color_schemes_field' ), 'sds-theme-options[general]', 'sds_theme_options_color_schemes_section' );
			}
		...
		?>
			<p>
				<?php
					...
					printf( __( 'Загрузите логотип это заменит текстовое название сайта. Рекомендуемые размеры: %1$s.', 'capture' ), $sds_logo_dimensions );
				?>
			</p>
		<?php
		}
		...
		function sds_theme_options_logo_field() {
			global $sds_theme_options;
		?>
			<strong><?php _e( 'Текущий Логотип:', 'capture' ); ?></strong>
			<div class="sds-theme-options-preview sds-theme-options-logo-preview">
				...
				?>
						<div class="description"><?php _e( 'Логотип не выбран.', 'capture' ); ?></div>
				<?php endif; ?>
			</div>
			...
			<input type="submit" id="sds_theme_options_logo_attach" class="button-primary sds-theme-options-upload" name="sds_theme_options_logo_attach"  value="<?php esc_attr_e( 'Выберите логотип', 'capture' ); ?>" data-media-title="Choose A Logo" data-media-button-text="Use As Logo" />
			<?php submit_button( __( 'Улалить логотип', 'capture' ), array( 'secondary', 'button-remove-logo' ), 'sds_theme_options[remove-logo]', false, ( ! isset( $sds_theme_options['logo_attachment_id'] ) || empty( $sds_theme_options['logo_attachment_id'] ) ) ? array( 'disabled' => 'disabled', 'data-init-empty' => 'true' ) : false ); ?>
		<?php
		}
		...
		?>
			<p><?php _e( 'Используйте эту опцию, чтобы показать или скрыть слоган сайта.', 'capture' ); ?></p>
		<?php
		}
		...
		?>
			<p><?php _e( 'Выберите цветовую схему для использования на Вашем сайте.', 'capture' ); ?></p>
		<?php
		}
		...
		?>
			<p><?php _e( 'Выберите веб-шрифт Google, чтобы использовать на вашем сайте.', 'capture' ); ?></p>
		<?php
		}
		...
		?>
			<div class="sbt-theme-options-web-fonts-wrap">
				<div class="sbt-theme-options-web-font sbt-theme-options-web-font-none">
						...
						<span class="sds-theme-options-web-font-label-none"><?php _e( 'None', 'capture' ); ?></span>
				</div>
		...
		?>
			<div class="wrap about-wrap">
				...
				<h3 class="nav-tab-wrapper sds-theme-options-tab-wrap">
					<a href="#general" id="general-tab" class="nav-tab sds-theme-options-tab nav-tab-active"><?php _e( 'General Options', 'capture' ); ?></a>
					...
					<a href="#help-support" id="help-support-tab" class="nav-tab sds-theme-options-tab"><?php _e( 'Help/Support', 'capture' ); ?></a>
				</h3>
