...
<section id="comments-container" class="comments-container post-comments post-comments-container cf <?php echo ( ( int ) get_comments_number() === 0 || ! have_comments() ) ? 'comments-container-no-comments' : 'comments-container-has-comments'; ?> <?php echo ( ! comments_open() && ! pings_open() ) ? 'comments-closed' : 'comments-open'; ?>">
	<section id="comments" class="comments-area <?php echo ( ( int ) get_comments_number() === 0 ) ? 'no-comments' : false; ?>">
		<?php if ( have_comments() ) : ?>
			<section class="comments-title-container cf">
				<h5 class="comments-title block-title">
					<?php
						printf( _n( 'Один комментарий на &ldquo;%2$s&rdquo;', '%1$s комментарии на &ldquo;%2$s&rdquo;', get_comments_number(), 'capture' ),
			...
			<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // Are there comments to navigate through? ?>
				<nav class="navigation comment-navigation cf" role="navigation">
					<section class="nav-previous"><?php previous_comments_link( __( '&larr; Предыдущие комментарии', 'capture' ) ); ?></section>
					<section class="nav-next"><?php next_comments_link( __( 'Следующие комментарии &rarr;', 'capture' ) ); ?></section>
				</nav>
		...
		<?php
			...
			comment_form( array(
				'title_reply' => __( 'Оставьте ваш комментарий', 'capture' ),
				'title_reply_to' => __( 'Оставьте комментарий на %s', 'capture' ),
				'cancel_reply_link' => __( 'Отменить комментарий', 'capture' ),
				'label_submit' => __( 'Отправить', 'capture' )
			) );
		?>
	</section>
</section>